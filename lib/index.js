...
var openingTag = function (s) {
  ...
  return regExpcape(s).replace(/@KEY/, [
    "(", sortedKeys(ANSI).join("|"), ")"
  ].join("")).replace(/\s+/, "\\s+");
};
...
var closingTag = function (s) {
  ...
  return regExpcape(s).replace(/\s+/, "\\s+");
};
...
var createANSI = function (CODES) {
  ...
  return _.reduce(CODES, function (o, v, k) {
    return _.extend(o, _.object([k], [
      [
        csisify(v[0]), csisify(v[1])
      ]
    ]));
  }, {});
};
...
var compressANSI = function (str) {
  ...
  s = _.chain(s).replaceAll(COMPRESS.shrink[0], COMPRESS.shrink[1]).replaceAll(COMPRESS.dedupe[0], COMPRESS.dedupe[1]).replaceAll(COMPRESS.trash1[0], COMPRESS.trash1[1]).replaceAll(COMPRESS.trash2[0], COMPRESS.trash2[1]).replaceAll(COMPRESS.trash3[0], COMPRESS.trash3[1]).replaceAll(COMPRESS.trash4[0], COMPRESS.trash4[1]).value();
